...
sm._upload_to_imgbb = lambda f: "http://img/" + f
shift = ShiftData()
shift.shift_id = s_id
shift.status = "closed"
shift.pancafe_screenshot_filename = "pan.png"
shift.form_screenshot_filename = "form.png"
...
with server_mod.app.test_request_context():
    ...
    check("pancafe written to col 23", wb.summary.cells.get((2, 23)) == "http://img/pan.png", str(wb.summary.cells))
    check("form written to col 24", wb.summary.cells.get((2, 24)) == "http://img/form.png", str(wb.summary.cells))
...
with server_mod.app.test_request_context():
    ...
    check("legacy pancafe col 23", wb2.summary.cells.get((2, 23)) == "http://img/pan.png", str(wb2.summary.cells))
    check("legacy form col 24", wb2.summary.cells.get((2, 24)) == "http://img/form.png", str(wb2.summary.cells))

# --- clear_sheets: resets synced flags on closed shifts ---
wb3 = Wb()
sm.client = type("C", (), {"open_by_key": lambda self, k: wb3})()
c1 = ShiftData()
c1.shift_id = "s1"; c1.status = "closed"; c1.synced_to_sheets = True
c2 = ShiftData()
c2.shift_id = "s2"; c2.status = "closed"; c2.synced_to_sheets = False
c3 = ShiftData()
c3.shift_id = "s3"; c3.status = "active"; c3.synced_to_sheets = True
saved = []
server_mod.get_all_shifts = lambda: [c1, c2, c3]
server_mod.save_shift = lambda s: saved.append(s.shift_id)

with server_mod.app.test_request_context():
    resp, code = call_route(server_mod.clear_sheets)
    body = resp.get_json()
    check("clear ok", code == 200 and body["success"], str(body))
    check("closed+synced reset", c1.synced_to_sheets is False, f"c1={c1.synced_to_sheets}")
    check("closed+unsynced stays", c2.synced_to_sheets is False)
    check("active untouched", c3.synced_to_sheets is True)
    check("only changed shifts saved", saved == ["s1"], str(saved))
